...
void SceneFactory::Initialize()
{
	...
	creationMap[SceneType::Hashino] = []() { return std::make_unique<HashinoDebugScene>(); }; // 橋野デバッグシーンを作成する
	creationMap[SceneType::Itou] = []() { return std::make_unique<ItouDebugScene>(); }; // 伊藤デバッグシーンを作成する
	creationMap[SceneType::Mizushima ] = []() { return std::make_unique<MizushimaDebugScene>(); }; // 水島デバッグシーンを作成する
	creationMap[SceneType::Tamura] = []() { return std::make_unique<TamuraDebugScene>(); }; // 田村デバッグシーンを作成する
}
